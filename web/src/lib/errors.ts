...
export function explainFailure(raw: string): Explained {
  ...
  if (/model.*(not found|does not exist)|unknown model|\b404\b/i.test(m)) return { what: "The writer model name is not offered by this endpoint.", todo: "In Settings, press Test and click one of the models it lists.", settings: true };
