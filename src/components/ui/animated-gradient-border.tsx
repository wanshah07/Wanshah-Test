...
// The rotation depends on three things living in src/index.css: an @property
// registration for --gradient-angle, the gradient-rotate keyframes, and the
// .gradient-border-* classes named below. Without the @property registration
// the angle is an untyped string and CSS cannot interpolate it, so the border
// renders but never moves.
...
interface BorderRotateProps extends Omit<HTMLAttributes<HTMLDivElement>, 'className'> {
  // Optional: the upstream type marked this required, but its own Default
  // example renders an empty frame — a bare decorative border is a valid use.
  children?: ReactNode;
  className?: string;
